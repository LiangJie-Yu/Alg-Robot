...
绿色:REF(频率可调，占空比也可调的PWM波形)
由右上角的图，可形象理解关于输出比较单元的CNT与CCR是如何控制REF的变化的，我们调整PWM波形的占空比，是通过==CCR的值的调控==的，如:CCR设置的高一些，输出的占空比就会变大，那么CCR设置的低一些，输出占空比就变小
### 参数计算
PWM频率：(等于计数器的更新频率)
**Freq = CK_PSC / (PSC + 1) / (ARR + 1)**
PWM占空比：
**Duty = CCR / (ARR + 1)**
PWM分辨率：(占空比变化的步距)(占空比变化的==越细腻就越好==)
**Reso = 1 / (ARR + 1)**
...
(上管导通，下管断开，输出为高电平)
(下管导通，上管断开，输出为低电平)
(上下管都导通，电源短路，不被允许，应尽量避免)
两个推挽电路：构成==H电桥==电路，用于==控制直流电机正反转==
三个MOS管用于==驱动三相无刷电机==
